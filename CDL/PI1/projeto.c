...
void Cadastrar() {
	...
	if (arq == NULL){
		printf("ERRO! O arquivo nao foi aberto!\n"); // Caso de algum erro no momento de abertura do arquivo, aparece essa menssagem.
	} else {
		...
		printf(" - Informe o usuario responsavel: ");
		scanf("%s", &responsavel);
...
void Alterar(){
	...
	if(linha_selecionada != 0){
		...
		scanf("%s", &versao);
		printf(" - Informe o novo usuario responsavel: ");
		scanf("%s", &responsavel);
